...
- 同一路線で残り日数が近い（±3日）過去価格の平均と当日価格を比較し、割安／割高を判定
...
flowchart TD
    ...
    User["ユーザー"] --> Worker
    D1 -->|"近い残り日数(±3日)の平均と比較"| Judge["割安／割高の判定"]
...
判定は、同一 `route` で当日の `days_before` を中心に **±3日の許容窓** に入る過去レコードの平均を出し、当日価格と比較して行います。当日のレコード自身は平均に含めないため、データが薄い時期に当日の高値が平均を引き上げる現象を避けています。

> 許容窓を設けるのは、毎日「次の4つの土曜」を収集する都合上 `days_before` が日々1ずつシフトし、完全一致だと同じ値が週1回しか溜まらないためです。±3日の窓で残り日数が近い観測をまとめ、サンプルを日次で蓄積します。
