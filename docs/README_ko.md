...
- 같은 노선·같은 남은 일수의 평균과 오늘 가격을 비교해 저렴/비쌈 판정
...
```mermaid
flowchart TD
    Cron["Cron Triggers<br/>매일 1회 (JST 12:00)"] --> Worker["Cloudflare Workers (Hono)"]
    Worker -->|"가격 취득 → JPY로 통일"| TP["Travelpayouts API"]
    TP --> Worker
    Worker -->|"노선·남은 일수·가격 축적"| D1[("D1 / SQLite")]
    User["사용자"] --> Worker
    D1 -->|"같은 남은 일수의 평균과 비교"| Judge["저렴／비쌈 판정"]
...
판정은 같은 `route`・같은 `days_before` 레코드의 평균을 내어 오늘 가격과 비교합니다. 오늘 레코드 자신은 평균에 포함하지 않아, 데이터가 적은 시기에 오늘의 높은 값이 평균을 끌어올리는 현상을 피합니다.
